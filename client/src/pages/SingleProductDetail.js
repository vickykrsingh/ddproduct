...
function SingleProductDetail() {
  ...
  const getProduct = async () => {
    setLoading(true);
    const { data } = await axios.get(`${process.env.REACT_APP_API_KEY}/api/v1/product/get-product/${pId}`);
    setLoading(false);
  ...
  const similarProduct = async (cId) => {
    try {
      const { data } = await axios.get(
        `${process.env.REACT_APP_API_KEY}/api/v1/product/similar-product/${pId}/${cId}`
      );
          ...
          <div className="row">
            <div className="col-md-6 p-3 col-lg-5">
              <img
                ...
                src={`${process.env.REACT_APP_API_KEY}/api/v1/product/product-photo/${detail._id}`}
                alt="Card_image_cap"
              />
            </div>
                    ...
                    {similarProducts.map((s) => (
                      ...
                      >
                        <img
                          ...
                          style={{ width: "15rem" }}
                          src={`${process.env.REACT_APP_API_KEY}/api/v1/product/product-photo/${s._id}`}
                          alt="Card_image_cap"
                        />
