...
function Cart() {
  ...
  const handleRemoveCart = async (event, id) => {
    ...
    try {
      const { data } = await axios.post(`${process.env.REACT_APP_API_KEY}/api/v1/cart/delete-single-cart`, {
        id,
      });
      if (data?.success) {
  ...
  const handleCheckout = async () => {
    try {
      const {
        data: { key },
      } = await axios.get(`${process.env.REACT_APP_API_KEY}/api/v1/payment/get-key`);
      console.log(key)
      const {
        data: { order },
      } = await axios.post(
        `${process.env.REACT_APP_API_KEY}/api/v1/payment/checkout`,
        {
          amount: Number(tot),
        }
      );
      ...
      var options = {
        ...
        handler: async function (response) {
          const { data } = await axios.post(
            `${process.env.REACT_APP_API_KEY}/api/v1/payment/payment-verification`,
            {
            ...
            <div className="col-12 p-3" key={p._id}>
              <div className="card d-flex flex-row p-2">
                <div className="card-image d-flex align-items-center justify-content-center">
                  <img
                    src={`${process.env.REACT_APP_API_KEY}/api/v1/product/product-photo/${p.productId}`}
                    alt="Apple watch"
                    width={"100px"}
                    className="rounded-2"
                  />
