...
function UpdateProduct() {
  ...
  const getProduct = async () => {
    try {
      const { data } = await axios.get(
        `${process.env.REACT_APP_API_KEY}/api/v1/product/get-product/${params.id}`
      );
  ...
  const fetchAllCategory = async () => {
    try {
      const { data } = await axios.get(`${process.env.REACT_APP_API_KEY}/api/v1/category/get-all-category`);
      if (data?.success) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${process.env.REACT_APP_API_KEY}/api/v1/product/update-product/${pid}`,
        productData
      );
      if (data?.success) {
        toast.success(data?.message);
        navigate("/dashboard/admin/products");
        await axios.get(`${process.env.REACT_APP_API_KEY}/api/v1/product/get-product`);
      } else {
  ...
  const handleDelete = async () => {
    try {
      ...
      const { data } = await axios.delete(
        `${process.env.REACT_APP_API_KEY}/api/v1/product/delete-product/${pid}`
      );
      if (data?.success) {
        toast.success(data?.message);
        navigate("/dashboard/admin/products");
            ...
            <div className="m-1 w-75">
              ...
              <div className="mb-3">
                {photo ? (
                  ...
                  <div className="text-center">
                    <img
                      src={`${process.env.REACT_APP_API_KEY}/api/v1/product/product-photo/${pid}`}
                      alt="product_photo"
                      height={"200px"}
                      className="img img-responsive"
                    />
