...
function CreateCategory() {
  ...
  const createCategoryHandler = async (e) => {
    e.preventDefault();
    try {
      const newCategory = await axios.post(`${process.env.REACT_APP_API_KEY}/api/v1/category/create-category`, {
        name,
      });
  ...
  const deleteCategoryHandler = async (id) => {
    try {
      const { data } = await axios.delete(
        `${process.env.REACT_APP_API_KEY}/api/v1/category/delete-category/${id}`
      );
      if (data?.success) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `${process.env.REACT_APP_API_KEY}/api/v1/category/update-category/${selected._id}`,
        { name: updated }
      );
      if (data?.success) {
