...
function AllOrders() {
  ...
  const handleChangeStatus = async (orderId, value) => {
    try {
      await axios.put(`${process.env.REACT_APP_API_KEY}/api/v1/payment/order-status/${orderId}`, {
        status: value,
      });
      getAllOrder();
    } catch (error) {
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post(`${process.env.REACT_APP_API_KEY}/api/v1/payment/search-order`, {
        searchKey: searchKey,
        searchValue: search,
      });
      if (data?.success) {
