...
export default function Home() {
  ...
  const getAllProduct = async (req, res) => {
    try {
      ...
      const { data } = await axios.get(`${process.env.REACT_APP_API_KEY}/api/v1/product/product-list/${page}`);
      setLoading(false);
  ...
  const fetchAllCategory = async () => {
    try {
      const category = await axios.get(`${process.env.REACT_APP_API_KEY}/api/v1/category/get-all-category`);
  ...
  const totalProductCount = async (req, res) => {
    try {
      const { data } = await axios.get(`${process.env.REACT_APP_API_KEY}/api/v1/product/product-count`);
      setTotal(data?.total);
    } catch (error) {
      toast.error("Request Timeout")
    }
  ...
  const filterProducts = async () => {
    try {
      const { data } = await axios.post(`${process.env.REACT_APP_API_KEY}/api/v1/product/product-filter`, {
        checked,
        radio,
      });
      setProducts(data?.products);
                        ...
                        products.map((p) => (
                          ...
                          >
                            <img
                              ...
                              src={`${process.env.REACT_APP_API_KEY}/api/v1/product/product-photo/${p._id}`}
                              alt="Card_image_cap"
                            />
